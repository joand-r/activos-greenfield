    ...
    const cargarDatos = async () => {
      ...
      try {
        ...
        if (oficina) {
          ...
          const data = await activoService.getAll({ lugar_id: oficina.id });
          setActivos(data);
        } else {
          setError('No se encontró el lugar de tipo Oficina');
        }
      } catch (err: any) {
        console.error('Error al cargar datos de oficina:', err);
