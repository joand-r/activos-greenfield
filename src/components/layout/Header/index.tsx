...
const Header = () => {
  ...
  return (
    <>
      <header
        className={`header top-0 z-35 flex w-full items-center transition-all ${
          isAuthenticated
            ? "sticky bg-white/95 dark:bg-black/95 border-b border-stroke dark:border-strokedark py-4 shadow-sm"
            : sticky
            ? "dark:bg-gray-dark dark:shadow-sticky-dark fixed z-[9999] bg-white !bg-opacity-80 shadow-sticky backdrop-blur-sm py-5 lg:py-2"
            : "absolute bg-transparent py-8"
        }`}
      >
        <div className={isAuthenticated ? "w-full px-6" : "container"}>
          <div className={`relative flex items-center justify-between ${isAuthenticated ? "" : "-mx-4"}`}>
            {!isAuthenticated && (
            ...
            <div className={`flex items-center justify-between px-4 ${isAuthenticated ? 'w-full justify-end' : 'w-full'}`}>
              {!isAuthenticated && (
                <div>
                  <button
              ...
              <div className={`flex items-center justify-end gap-4 ${isAuthenticated ? "pr-4" : "pr-16 lg:pr-0"}`}>
                {isAuthenticated ? (
                  <>
